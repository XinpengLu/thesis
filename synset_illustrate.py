...
def load_critic(env, critic_path):
    critic = FullyConnectedQFunction(env.observation_space.shape[0], env.action_space.shape[0])
...
def load_policy(env,policy_path):
    policy = TanhGaussianPolicy(env.observation_space.shape[0], env.action_space.shape[0], float(env.action_space.high[0]), orthogonal_init=True)
...
syn_dset_size = 256
offline_policy_path = os.path.join('./offline_policy_checkpoints', 'Cal-QL-' + config.dataset.name,
                                   'checkpoint.pt')
...
policy = load_policy(env, config.offline_policy_path)
